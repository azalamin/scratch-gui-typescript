 ...
 */
const extractFileName = function (nameExt: any): any {
...
const handleFileUpload = function (
  ...
  const readFile = (i: any, files: any) => {
    if (i === files.length) {
...
const costumeUpload = function (
  ...
  switch (fileType) {
    ...
    case 'image/gif': {
      let costumes: any = [];
      gifDecoder(fileData, (frameNumber: any, dataUrl: any, numFrames: any) => {
        costumeUpload(
          dataUrl,
          'image/png',
          storage,
  ...
  const addCostumeFromBuffer = function (dataBuffer: any) {
