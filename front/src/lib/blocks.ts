...
export default function (vm: any): any {
  ...
  const jsonForSensingMenus = function (menuOptionsFn: any) {
    return {
  ...
  const soundsMenu = function () {
    ...
    if (vm.editingTarget && vm.editingTarget.sprite.sounds.length > 0) {
      menu = vm.editingTarget.sprite.sounds.map((sound: any) => [
        sound.name,
        sound.name,
      ]);
    }
  ...
  const costumesMenu = function () {
    if (vm.editingTarget && vm.editingTarget.getCostumes().length > 0) {
      return vm.editingTarget
        .getCostumes()
        .map((costume: any) => [costume.name, costume.name]);
    }
    return [['', '']];
  };
  ...
  ScratchBlocks.VerticalFlyout.getCheckboxState = function (blockId: any) {
  ...
  ScratchBlocks.FieldNote.playNote_ = function (
    noteNum: any,
    extensionId: any
  ) {
  ...
  ScratchBlocks.scratchBlocksUtils.compareStrings = function (
    str1: any,
    str2: any
  ) {
