 ...
 */
const indexForPositionOnList = ({ x, y }: any, boxes: any, isRtl: any): any => {
  if (boxes.length === 0) return null;
  let index = null;
  const leftEdge = Math.min.apply(
    null,
    boxes.map((b: any) => b.left)
  );
  const rightEdge = Math.max.apply(
    null,
    boxes.map((b: any) => b.right)
  );
  const topEdge = Math.min.apply(
    null,
    boxes.map((b: any) => b.top)
  );
  const bottomEdge = Math.max.apply(
    null,
    boxes.map((b: any) => b.bottom)
  );
