...
const SortableHOC: any = function (WrappedComponent: any) {
  class SortableWrapper extends React.Component<PropsInterface> {
    ...
    componentWillReceiveProps(newProps: any) {
      if (newProps.dragInfo.dragging && !this.props.dragInfo.dragging) {
        ...
        this.boxes = this.sortableRefs.map(
          (el: any) => el && el.getBoundingClientRect()
        );
        this.boxes.sort((a: any, b: any) => {
    ...
    handleAddSortable(node: any) {
      this.sortableRefs.push(node);
    }

    handleRemoveSortable(node: any) {
    ...
    getOrdering(items: any, draggingIndex: any, newIndex: any) {
    ...
    getMouseOverIndex() {
      ...
      if (this.props.dragInfo.currentOffset) {
        const { x, y }: any = this.props.dragInfo.currentOffset;
